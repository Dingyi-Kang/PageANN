...
{
    std::string index_prefix_path, data_type, dist_fn, base_data_file, full_ooc_str, new_to_old_ids_map_file;
    uint32_t min_degree_per_node, num_pq_chunks_32, maxVamanaDegree;
    bool full_ooc = false;
    bool enable_spare_fill = true;
    ...
    try
    {
        ...
        desc.add_options()("new_to_old_ids_map_file", po::value<std::string>(&new_to_old_ids_map_file)->default_value(""),
            "Path to pre-computed new_to_old_ids_map.bin. If provided, skips mergeNodesIntoMegaNodes and reuses the existing page grouping");
        desc.add_options()("enable_spare_fill", po::value<bool>(&enable_spare_fill)->default_value(true),
            "Enable beam-search spare-space filling in create_pageann_disk_layout (default: true). Set false to disable.");
        po::variables_map vm;
    ...
    try
    {
        if (data_type == std::string("float"))
             diskann::build_page_graph<float>(index_prefix_path, base_data_file, min_degree_per_node, maxVamanaDegree, num_pq_chunks_32, metric, memBudgetInGB, full_ooc, new_to_old_ids_map_file, enable_spare_fill);
        if (data_type == std::string("int8"))
             diskann::build_page_graph<int8_t>(index_prefix_path, base_data_file, min_degree_per_node, maxVamanaDegree, num_pq_chunks_32, metric, memBudgetInGB, full_ooc, new_to_old_ids_map_file, enable_spare_fill);
        if (data_type == std::string("uint8"))
             diskann::build_page_graph<uint8_t>(index_prefix_path, base_data_file, min_degree_per_node, maxVamanaDegree, num_pq_chunks_32, metric, memBudgetInGB, full_ooc, new_to_old_ids_map_file, enable_spare_fill);
    }
