...
std::vector<std::vector<std::pair<uint32_t, float>>> processUnfilteredParts(const std::string &base_file,
                                                                            ...
                                                                            std::vector<uint32_t> &original_to_new_ids)
{
    ...
    for (int p = 0; p < num_parts; p++)
    {
        ...
        for (size_t i = 0; i < nqueries; i++)
        {
            ...
            {
                if (!original_to_new_ids.empty())
                    if (original_to_new_ids[closest_points_part[i * k + j] + start_id] == 0)
                        continue;
...
int aux_main(const std::string &base_file, const std::string &query_file, const std::string &gt_file, size_t k,
             const diskann::Metric &metric, const std::string &id_map_file, const std::string &vamana_gt_file,
             const bool skip_knn)
{
    size_t npoints, nqueries, dim;
    float *query_data;

    const bool tags_enabled = !id_map_file.empty();
    std::vector<uint32_t> original_to_new_ids;
    if (tags_enabled)
    {
        original_to_new_ids = diskann::loadTags(id_map_file, base_file);
    }
    std::cout << "Size of ids map: " << original_to_new_ids.size() << std::endl;
    ...
    if (skip_knn && tags_enabled && !vamana_gt_file.empty()){
        ...
        for (size_t i = 0; i < nqueries; i++)
        {
            for(size_t j = 0; j < k; j++){
                //closest_points[i * k + j] = original_to_new_ids[original_closest_points[i * k + j]];
                auto id = original_closest_points[i * k + j];
                if (id >= original_to_new_ids.size()) {
                    std::cerr << "Error: id " << id << " out of range (max "
                           << original_to_new_ids.size()-1 << ")\n";
                    return -1;
                }
                closest_points[i * k + j] = original_to_new_ids[id];
            }
        }
        ...
        std::vector<std::vector<std::pair<uint32_t, float>>> results =
            processUnfilteredParts<T>(base_file, nqueries, npoints, dim, k, query_data, metric, original_to_new_ids);
        ...
        for (size_t i = 0; i < nqueries; i++)
        {
            ...
            for (auto iter : cur_res)
            {
                ...
                if (tags_enabled)
                {
                    ...
                    closest_points[i * k + j] = original_to_new_ids[iter.first];
                }
                else
                {
...
{
    std::string data_type, dist_fn, base_file, query_file, gt_file, id_map_file, vamana_gt_file;
    uint64_t K;
    bool skip_knn = false;
    ...
    try
    {
        ...
        desc.add_options()("id_map_file", po::value<std::string>(&id_map_file)->default_value(std::string()),
                           "Path to the original-to-new IDs mapping file");
        ...
        desc.add_options()("skip_knn", po::bool_switch(&skip_knn)->default_value(false),
                           "Skip KNN computation and only convert IDs from vamana_gt_file");
        ...
        // Validate: if skip_knn is set, vamana_gt_file and id_map_file are required
        if (skip_knn && (vamana_gt_file.empty() || id_map_file.empty()))
        {
            std::cerr << "Error: --vamana_gt_file and --id_map_file are required when --skip_knn is set" << std::endl;
            return -1;
        }
    }
    ...
    try
    {
        if (data_type == std::string("float"))
            aux_main<float>(base_file, query_file, gt_file, K, metric, id_map_file, vamana_gt_file, skip_knn);
        if (data_type == std::string("int8"))
            aux_main<int8_t>(base_file, query_file, gt_file, K, metric, id_map_file, vamana_gt_file, skip_knn);
        if (data_type == std::string("uint8"))
            aux_main<uint8_t>(base_file, query_file, gt_file, K, metric, id_map_file, vamana_gt_file, skip_knn);
    }
