...
#ifdef __linux__
// Forward declaration for Linux async I/O
class LinuxAsyncFileReader;
#endif

// Async I/O support structures
struct IOBuffer {
    char *data = nullptr;
    size_t size = 0;
    bool in_use = false;
...
template <typename T> class SSDQueryScratch : public AbstractScratch<T>
{
  public:
    ...
    //only for pageann
    NeighborPriorityQueue nav_retset;  // Navigation graph search retset
    std::vector<Neighbor> full_retset;
    //only for laann
    tsl::robin_set<uint32_t> seededPages;            // Phase 1.5 nav seeding dedup (max nav_L entries)
    std::vector<uint32_t> deferred_proc_vec_uncached_page_ids;  // page IDs whose full-precision distance is deferred
    std::vector<char>     deferred_proc_uncached_vec_buffer;    // vector data copied from I/O buffers before release
    NeighborPriorityQueue full_ret_queue;

    bool use_pageann = true; 

    // Async I/O state (persists across queries within this thread)
#ifdef __linux__
    LinuxAsyncFileReader *async_reader = nullptr;  // LAANN: Async I/O reader (Linux only)
    // LAANN: I/O buffer pool for async I/O
    std::vector<IOBuffer> io_buffer_pool;
    // LAANN: Async I/O tracking maps
    std::unordered_map<uint32_t, char*> completed_io_map;  // nodeID -> buffer (completed reads)
    std::unordered_map<uint32_t, char*> pending_io_map;    // megaNodeID -> buffer (submitted but not completed)s
#endif

    SSDQueryScratch(size_t aligned_dim, size_t estimated_visited_vectors, size_t nnodes_sector, bool use_pageann = true, uint32_t window_search_W = 8);
    ~SSDQueryScratch();
...
template <typename T> class SSDThreadData
{
  public:
    ...
    SSDThreadData(size_t aligned_dim, size_t estimated_visited_vectors, size_t nnodes_sector, bool use_pageann = true, uint32_t window_search_W = 8);
    void clear();
};
