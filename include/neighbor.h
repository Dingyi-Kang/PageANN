...
class NeighborPriorityQueue
{
  public:
    ...
    // Get current position (first unexpanded index)
    size_t current_position() const
    {
        return _cur;
    }

    // Update _cur to point to first unexpanded
    void update_cur_to_next_unexpanded()
    {
        while (_cur < _size && _data[_cur].expanded)
        {
            _cur++;
    ...
    // Reset all expanded flags and cursor for reuse (unified search transition)
    void reset_expansion()
    {
        for (size_t i = 0; i < _size; ++i)
        {
            _data[i].expanded = false;
        }
        _cur = 0;
    }
