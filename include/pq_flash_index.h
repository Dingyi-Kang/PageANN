...
template <typename T, typename LabelT = uint32_t> class PQFlashIndex
{
  public:
     ...
     * @note Navigation graph must be loaded separately via load_nav_graph() if desired
     */
    DISKANN_DLLEXPORT int load(uint32_t num_threads, const char *index_prefix, const std::string &pq_path_prefix);
#endif
    ...
    /**
     * @brief Load navigation graph for entry point selection.
     *
     * Loads the in-memory navigation graph built from sampled vectors.
     * The navigation graph is used to find better entry points for page search.
     *
     * @param index_prefix Path prefix for index files (navigation graph file is {prefix}_nav_graph)
     * @return 0 on success, -1 on failure
     *
     * @note Call this after load() if you want to use navigation graph routing
     * @note Pass nav_L > 0 during search to enable navigation graph routing
     */
    DISKANN_DLLEXPORT void load_nav_graph(const char *index_prefix);
     ...
     * @param nav_L Navigation graph search L (0 = don't use nav graph, default = 0)
     *
     ...
     * @note If nav_L > 0 and navigation graph is loaded, uses nav graph routing
     */
    DISKANN_DLLEXPORT void generate_cache_list_from_sample_queries(std::string sample_bin, uint64_t l_search,
                                                                    ...
                                                                    uint32_t nthreads,
                                                                    std::vector<uint32_t> &page_list, const uint32_t nav_L = 0);
#endif
     ...
     * 2. Select entry point(s) using navigation graph (if loaded) or medoid nodes
    ...
    DISKANN_DLLEXPORT void page_search(const T *query, const uint64_t k_search, const uint64_t l_search,
                                              ...
                                              const bool use_reorder_data = false, QueryStats *stats = nullptr, const uint32_t nav_L = 0);

    /** @brief page_search overload with label filtering support (if nav_L > 0 and navigation graph is loaded, uses nav graph routing) */
    DISKANN_DLLEXPORT void page_search(const T *query, const uint64_t k_search, const uint64_t l_search,
                                              uint64_t *res_ids, float *res_dists, const uint64_t beam_width,
                                              const bool use_filter, const LabelT &filter_label,
                                              const bool use_reorder_data = false, QueryStats *stats = nullptr, const uint32_t nav_L = 0);

    /** @brief page_search overload with I/O limit to cap disk operations (if nav_L > 0 and navigation graph is loaded, uses nav graph routing) */
    DISKANN_DLLEXPORT void page_search(const T *query, const uint64_t k_search, const uint64_t l_search,
                                              uint64_t *res_ids, float *res_dists, const uint64_t beam_width,
                                              const uint32_t io_limit, const bool use_reorder_data = false,
                                              QueryStats *stats = nullptr, const uint32_t nav_L = 0);

    /** @brief page_search overload with both label filtering and I/O limit (if nav_L > 0 and navigation graph is loaded, uses nav graph routing) */
    DISKANN_DLLEXPORT void page_search(const T *query, const uint64_t k_search, const uint64_t l_search,
                                              uint64_t *res_ids, float *res_dists, const uint64_t beam_width,
                                              const bool use_filter, const LabelT &filter_label,
                                              const uint32_t io_limit, const bool use_reorder_data = false,
     ...
     * 2. Entry point selection (navigation graph if loaded, otherwise medoid)
    ...
    DISKANN_DLLEXPORT uint64_t get_num_pages();
    DISKANN_DLLEXPORT uint64_t get_num_cached_pages();

  protected:
    DISKANN_DLLEXPORT void use_medoids_data_as_centroids();
    DISKANN_DLLEXPORT void setup_thread_data(uint64_t nthreads, uint64_t estimated_visited_vectors = 2048);
  ...
  private:
    ...
    // Navigation graph routing data structures (flat array for efficiency)
    T* _nav_data = nullptr;                    // Sampled vector data for navigation
    uint32_t* _nav_graph = nullptr;            // Flat adjacency list [node0_nbrs, node1_nbrs, ...]
    uint8_t* _nav_num_neighbors = nullptr;     // Actual neighbor count per node
    uint32_t _nav_start = 0;                   // Entry point node
    uint32_t _nav_num_points = 0;              // Number of samples in nav graph
    uint32_t _nav_aligned_dim = 0;             // Aligned dimension for nav data
    uint32_t _nav_max_degree = 0;              // Maximum degree (for offset: node_id * max_degree)
    uint32_t _samples_per_page = 1;            // Vectors sampled per page in priority order (normal mode)
    uint32_t _nav_total_pages = 0;            // Total pages in index; >0 means page subsampling mode
    uint32_t _nav_nnodes_per_page = 0;        // Vectors per page
    std::vector<uint32_t> _nav_sample_indices; // Priority-order offsets within a page: [0, last, second-to-last, ...]
    bool _use_nav_graph = false;              // Whether nav graph is loaded
