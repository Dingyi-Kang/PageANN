...
template <typename T, typename LabelT> PQFlashIndex<T, LabelT>::~PQFlashIndex()
{
    ...
    // Manual cleanup: Navigation graph data
    if (_nav_data != nullptr){
        diskann::aligned_free(_nav_data);
        _nav_data = nullptr;
    }
    if (_nav_graph != nullptr){
        delete[] _nav_graph;
        _nav_graph = nullptr;
    }
    if (_nav_num_neighbors != nullptr){
        delete[] _nav_num_neighbors;
        _nav_num_neighbors = nullptr;
    }
...
template <typename T, typename LabelT>
void PQFlashIndex<T, LabelT>::setup_thread_data(uint64_t nthreads, uint64_t estimated_visited_vectors)
{
...
#pragma omp critical
        {
            SSDThreadData<T> *data = new SSDThreadData<T>(this->_aligned_dim, estimated_visited_vectors, _nnodes_per_sector);//each data includes a SSDQueryScratch and one IOContext
...
template <typename T, typename LabelT> void PQFlashIndex<T, LabelT>::load_cache_list(std::vector<uint32_t> &page_list)
{
    ...
    size_t num_blocks = DIV_ROUND_UP(num_cached_pages, CACHE_BLOCK_SIZE);
    for (size_t block = 0; block < num_blocks; block++)
    {
        size_t start_idx = block * CACHE_BLOCK_SIZE;
        size_t end_idx = (std::min)(num_cached_pages, (block + 1) * CACHE_BLOCK_SIZE);
...
void PQFlashIndex<T, LabelT>::generate_cache_list_from_sample_queries(std::string sample_bin, uint64_t l_search,
                                                                      ...
                                                                      uint32_t nthreads,
                                                                      std::vector<uint32_t> &page_list, const uint32_t nav_L)
{
#endif
    if(!_all_pq_cached){
    ...
    // Note: even when num_pages_to_cache >= _num_pages we still profile and sort,
    // so the saved order file reflects true visit frequency for all pages.
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
    for (int64_t i = 0; i < (int64_t)sample_num; i++)
    {
        ...
        linux_page_search(samples + (i * sample_aligned_dim), 1, l_search, query_result_ids.data() + i * 1,
                         query_result_dists.data() + i * 1, beamwidth, false, stats, nav_L);
    }
    ...
    // Write all pages in frequency order so the saved file covers the full dataset.
    // The caller trims to the actual num_nodes_to_cache when loading at search time.
    uint32_t pages_to_write = static_cast<uint32_t>(this->_page_visit_counter.size());
    page_list.clear();
    page_list.shrink_to_fit();
    page_list.reserve(pages_to_write);
...
{
#else
template <typename T, typename LabelT> int PQFlashIndex<T, LabelT>::load(uint32_t num_threads, const char *index_prefix, const std::string &pq_path_prefix)
{
#endif
    ...
    return load_from_separate_paths(num_threads, index_prefix, pq_table_bin.c_str(),
                                    pq_compressed_reorder_file.c_str());
#endif
}
...
int PQFlashIndex<T, LabelT>::load_from_separate_paths(uint32_t num_threads, const char *index_filepath,
                                                      const char *pivots_filepath, const char *pq_compressed_reorder_file_path)
{
#endif
...
template <typename T, typename LabelT>
void PQFlashIndex<T, LabelT>::load_nav_graph(const char *index_prefix)
{
    std::string nav_graph_file = std::string(index_prefix) + "_nav_graph.index";
    std::string nav_data_file = std::string(index_prefix) + "_nav_data.bin";

    // Check if files exist
    std::ifstream nav_graph_test(nav_graph_file);
    std::ifstream nav_data_test(nav_data_file);

    if (!nav_graph_test.good() || !nav_data_test.good()) {
        diskann::cerr << "Navigation graph files not found, skipping" << std::endl;
        _use_nav_graph = false;
        return;
    }
    nav_graph_test.close();
    nav_data_test.close();

    diskann::cout << "Loading navigation graph from: " << nav_graph_file << std::endl;

    try {
        // Load graph structure (Starling format)
        std::ifstream graph_in(nav_graph_file, std::ios::binary);

        // Read header
        uint64_t expected_file_size;
        graph_in.read((char*)&expected_file_size, sizeof(uint64_t));
        graph_in.read((char*)&_nav_num_points, sizeof(uint32_t));     // Number of nodes
        graph_in.read((char*)&_nav_max_degree, sizeof(uint32_t));     // Max degree
        graph_in.read((char*)&_nav_start, sizeof(uint32_t));          // Entry point
        graph_in.read((char*)&_samples_per_page,    sizeof(uint32_t));  // samples per page (normal mode)
        graph_in.read((char*)&_nav_nnodes_per_page, sizeof(uint32_t));  // vectors per page
        // 7th field: num_total_pages — >0 means page subsampling mode.
        _nav_total_pages = 0;
        graph_in.read((char*)&_nav_total_pages, sizeof(uint32_t));

        // Pre-compute priority-order sample indices within a page for normal mode.
        // Sampling order: top(0), last, second-to-last, ...
        _nav_sample_indices.clear();
        _nav_sample_indices.push_back(0);
        for (uint32_t i = 1; i < _nav_nnodes_per_page && _nav_sample_indices.size() < _samples_per_page; ++i)
            _nav_sample_indices.push_back(_nav_nnodes_per_page - i);

        diskann::cout << "  Graph header:" << std::endl;
        diskann::cout << "    Num points: " << _nav_num_points << std::endl;
        diskann::cout << "    Max degree: " << _nav_max_degree << std::endl;
        diskann::cout << "    Entry point: " << _nav_start << std::endl;
        diskann::cout << "    Nodes per page: " << _nav_nnodes_per_page << std::endl;
        if (_nav_total_pages > 0)
            diskann::cout << "    Mode: page subsampling (" << _nav_num_points
                         << " pages out of " << _nav_total_pages << ")" << std::endl;
        else
            diskann::cout << "    Mode: " << _samples_per_page << " sample(s)/page" << std::endl;

        // Allocate flat arrays directly (no counting pass needed!)
        _nav_graph = new uint32_t[_nav_num_points * _nav_max_degree];
        _nav_num_neighbors = new uint8_t[_nav_num_points];
        memset(_nav_graph, 0, _nav_num_points * _nav_max_degree * sizeof(uint32_t));
        memset(_nav_num_neighbors, 0, _nav_num_points * sizeof(uint8_t));

        // Load adjacency lists into flat array
        for (uint32_t node_id = 0; node_id < _nav_num_points; node_id++) {
            uint32_t num_neighbors;
            graph_in.read((char*)&num_neighbors, sizeof(uint32_t));

            if (num_neighbors > 255) {
                diskann::cerr << "Warning: node " << node_id
                             << " has " << num_neighbors
                             << " neighbors, clamping to 255" << std::endl;
                num_neighbors = 255;
            }

            _nav_num_neighbors[node_id] = static_cast<uint8_t>(num_neighbors);

            // Read neighbors into flat array at fixed offset
            uint32_t* neighbors_start = _nav_graph + node_id * _nav_max_degree;
            graph_in.read((char*)neighbors_start, num_neighbors * sizeof(uint32_t));
        }

        graph_in.close();

        // Load vector data
        size_t nav_num_points_size_t, nav_dim;
        diskann::get_bin_metadata(nav_data_file, nav_num_points_size_t, nav_dim);

        // Verify consistency
        if (nav_num_points_size_t != _nav_num_points) {
            diskann::cerr << "Warning: nav data file has different number of points! "
                      << "Expected: " << _nav_num_points
                      << ", Got: " << nav_num_points_size_t << std::endl;
        }

        _nav_aligned_dim = ROUND_UP(nav_dim, 8);

        diskann::alloc_aligned((void**)&_nav_data,
                              _nav_num_points * _nav_aligned_dim * sizeof(T),
                              8 * sizeof(T));

        size_t nav_num_points_load = _nav_num_points;
        size_t nav_dim_load = nav_dim;
        size_t nav_aligned_dim_load = _nav_aligned_dim;
        diskann::load_aligned_bin<T>(nav_data_file, _nav_data, nav_num_points_load, nav_dim_load, nav_aligned_dim_load);

        diskann::cout << "Navigation graph loaded successfully:" << std::endl;
        diskann::cout << "  Points: " << _nav_num_points << std::endl;
        diskann::cout << "  Dimension: " << nav_dim << " (aligned: " << _nav_aligned_dim << ")" << std::endl;
        diskann::cout << "  Samples per page: " << _samples_per_page << std::endl;

        _use_nav_graph = true;

    } catch (const std::exception& e) {
        diskann::cerr << "Error loading navigation graph: " << e.what() << std::endl;
        diskann::cerr << "Falling back to medoid-based routing" << std::endl;
        _use_nav_graph = false;
        if (_nav_data != nullptr) {
            diskann::aligned_free(_nav_data);
            _nav_data = nullptr;
                                                 ...
                                                 const bool use_reorder_data, QueryStats *stats, const uint32_t nav_L)
{
    page_search(query1, k_search, l_search, indices, distances, beam_width, use_filter, filter_label,
                       std::numeric_limits<uint32_t>::max(), use_reorder_data, stats, nav_L);
}
...
void PQFlashIndex<T, LabelT>::page_search(const T *query1, const uint64_t k_search, const uint64_t l_search,
                                                 ...
                                                 QueryStats *stats, const uint32_t nav_L)
{
                                                 ...
                                                 QueryStats *stats, const uint32_t nav_L)
{
    ...
    // Navigation graph-based entry point selection (same convergence as disk search) _use_nav_graph is true meaning related data has been successfully loaded too
    if (nav_L > 0 && _use_nav_graph){
        useMediod = false;

        // 1. Initialize navigation graph retset with entry point
        NeighborPriorityQueue &nav_retset = query_scratch->nav_retset;  // Use from scratch
        nav_retset.reserve(nav_L);

        float dist_to_start = _dist_cmp->compare(aligned_query_T,
                                                 _nav_data + _nav_start * _nav_aligned_dim,
                                                 _nav_aligned_dim);
        nav_retset.insert(Neighbor(_nav_start, dist_to_start));

        // 2. Greedy best-first search on navigation graph (converge like disk search)
        while (nav_retset.has_unexpanded_node()) {
            auto current = nav_retset.closest_unexpanded();  // Auto marks as expanded
            uint32_t node_id = current.id;

            // Get neighbors from flat array
            uint32_t* neighbors_start = _nav_graph + node_id * _nav_max_degree;
            uint8_t num_neighbors = _nav_num_neighbors[node_id];

            // Explore neighbors
            for (uint8_t i = 0; i < num_neighbors; i++) {
                uint32_t neighbor_id = neighbors_start[i];

                float dist = _dist_cmp->compare(aligned_query_T,
                                               _nav_data + neighbor_id * _nav_aligned_dim,
                                               _nav_aligned_dim);

                nav_retset.insert(Neighbor(neighbor_id, dist));
            }
        }

        // 3. Transfer top nav_L results to disk search retset
        //    Convert sample indices → vector IDs, use full-precision distances
        size_t num_candidates = std::min((size_t)nav_L, nav_retset.size());

        for (size_t i = 0; i < num_candidates; ++i) {
            uint32_t sample_idx = nav_retset[i].id;
            float full_prec_dist = nav_retset[i].distance;

            // Convert nav sample_idx to the original vector ID.
            // Page subsampling mode: selected_page[i] = i*(N-1)/(K-1), top vector of that page.
            // Normal mode: page_id = sample_idx / samples_per_page,
            //   offset within page = sample_indices[sample_idx % samples_per_page].
            uint32_t vector_id;
            if (_nav_total_pages > 0) {
                uint32_t K = _nav_num_points, N = _nav_total_pages;
                uint32_t selected_page = (K <= 1) ? 0
                    : static_cast<uint32_t>((uint64_t)sample_idx * (N - 1) / (K - 1));
                vector_id = selected_page * _nav_nnodes_per_page;
            } else {
                uint32_t page_id = sample_idx / _samples_per_page;
                uint32_t offset  = sample_idx % _samples_per_page;
                vector_id = page_id * _nav_nnodes_per_page +
                            (offset < _nav_sample_indices.size() ? _nav_sample_indices[offset] : 0);
            }

            // Insert into disk search retset with full-precision distance
            retset.insert(Neighbor(vector_id, full_prec_dist));
        }

        if (retset.size() == 0) useMediod = true;  // Fallback
    }
    ...
    tsl::robin_set<uint32_t> disk_read_pages;
    ...
    while (retset.has_unexpanded_node() && num_ios < io_limit)
    {
        ...
        if (!frontier.empty())
        {
            ...
            if (stats != nullptr)
            {
                stats->tail_io_us += (float)io_timer.elapsed();
            }
        }
                                                ...
                                                const bool use_reorder_data, QueryStats *stats, const uint32_t nav_L)
{
    ...
    // Navigation graph-based entry point selection (same convergence as disk search)
    if (nav_L > 0 && _use_nav_graph){
        useMediod = false;
        ...
        /// MARK: option 2
        // // Get PQ coordinate for start node and compute distance
        // uint32_t start_vector_id = _nav_start * _sampling_interval;
        // uint32_t start_pq_idx = start_vector_id;
        // if (_useID_pqIdx_map) start_pq_idx = _nodeID_pqIdx_map[start_vector_id];
        // else if (_useID_pqIdx_array) start_pq_idx = _nodeID_pqIdx_arr[start_vector_id];

        // memcpy(pq_coord_scratch, _cached_pq_buff + start_pq_idx * _n_chunks, _n_chunks * sizeof(uint8_t));
        // diskann::pq_dist_lookup(pq_coord_scratch, 1, _n_chunks, pq_dists, dist_scratch);
        // float dist_to_start = dist_scratch[0];

        // nav_retset.insert(Neighbor(_nav_start, dist_to_start));

        // // 2. Greedy best-first search on navigation graph using PQ distances
        // while (nav_retset.has_unexpanded_node()) {
        ...
        //     // Batch load PQ coordinates for all neighbors
        //     uint8_t* pq_buf_ptr = pq_coord_scratch;
        //     for (uint8_t i = 0; i < num_neighbors; i++) {
        //         uint32_t neighbor_id = neighbors_start[i];
        //         uint32_t neighbor_vector_id = neighbor_id * _sampling_interval;

        //         uint32_t pq_idx = neighbor_vector_id;
        //         if (_useID_pqIdx_map) pq_idx = _nodeID_pqIdx_map[neighbor_vector_id];
        //         else if (_useID_pqIdx_array) pq_idx = _nodeID_pqIdx_arr[neighbor_vector_id];

        //         memcpy(pq_buf_ptr, _cached_pq_buff + pq_idx * _n_chunks, _n_chunks * sizeof(uint8_t));
        //         pq_buf_ptr += _n_chunks;
        //     }

        //     // Batch compute PQ distances
        //     diskann::pq_dist_lookup(pq_coord_scratch, num_neighbors, _n_chunks, pq_dists, dist_scratch);

        //     // Insert neighbors with PQ distances
        //     for (uint8_t i = 0; i < num_neighbors; i++) {
        //         uint32_t neighbor_id = neighbors_start[i];
    ...
    while (retset.has_unexpanded_node() && num_ios < io_limit)
    {
        ...
        Timer prev_pages_processing_timer;
        ...
        if (stats != nullptr) {
            stats->cpu_us += (float)prev_pages_processing_timer.elapsed();
        }

        last_pages_ids.clear();

        // process cached nhoods of each cached pages
        Timer cached_processing_timer;
        for (auto &cached_nhood : cached_nhoods)
        {
            const uint32_t pageID = cached_nhood.first;
            const uint32_t first_node_id = static_cast<uint32_t>(pageID * _nnodes_per_sector);
        ...
        if (stats != nullptr) {
            stats->cpu_us += (float)cached_processing_timer.elapsed();
        }

        // Process frontier pages
        Timer frontier_processing_timer;
        for (auto &frontier_nhood : frontier_nhoods)
        {
        ...
        if (stats != nullptr) {
            stats->cpu_us += (float)frontier_processing_timer.elapsed();
        }
    ...
    // Process final pages
    Timer final_pages_processing_timer;
    for (size_t i = 0; i < last_pages_ids.size(); ++i)
    {
        uint32_t pageID = last_pages_ids[i];
        char *page_data_buf = last_pages_buffer.data() + i * all_full_vector_space;
    ...
    if (stats != nullptr) {
        stats->cpu_us += (float)final_pages_processing_timer.elapsed();
    }
...
template <typename T, typename LabelT> std::uint64_t PQFlashIndex<T, LabelT>::get_num_pages()
{
    return _num_pages;
}

template <typename T, typename LabelT> std::uint64_t PQFlashIndex<T, LabelT>::get_num_cached_pages()
{
    return _cached_page_idx_map.size();
}
